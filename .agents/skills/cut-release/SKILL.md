...
4. Read `C:/worktrees-SeekOut/workbench/config.yaml` for repo list. Ask via AskUserQuestion which repo to use.
...
```bash
cd "C:/worktrees-SeekOut/<repo>.git" && echo "y" | bash C:/worktrees-SeekOut/workbench/scripts/windows/wt-release.sh [prefix] --repo <repo> [--source <branch>] [--date <date>] [--dry-run]
```
